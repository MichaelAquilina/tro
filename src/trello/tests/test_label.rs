...
fn test_get_all() -> Result<()> {
    let _m = mockito::mock("GET", "/1/boards/123-456/labels?fields=id%2Cname%2Ccolor")
        .match_header(
            "authorization",
...
fn test_apply() -> Result<()> {
    let _m = mockito::mock("POST", "/1/cards/SOME-CARD-ID/idLabels")
        .match_header(
            "authorization",
            "OAuth oauth_consumer_key=\"some-key\", oauth_token=\"some-token\"",
        )
...
fn test_remove() -> Result<()> {
    let _m = mockito::mock("DELETE", "/1/cards/FOO-CARD/idLabels/BAR-LABEL")
        .match_header(
            "authorization",
            "OAuth oauth_consumer_key=\"some-key\", oauth_token=\"some-token\"",
        )
